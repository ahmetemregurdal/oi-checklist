...
export async function google(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { code?: string; state?: string } }>('/google/callback', async (req, res) => {
    ...
    const baseUsername = userInfo.email.split('@', 1)[0].toLowerCase();
    const displayName = userInfo.name || userInfo.email || 'Google User';
    const authIdentity = await db.authIdentity.findFirst({ where: { provider, providerUserId } });
    let user: User;
    if (authIdentity) { // already linked to an account
      ...
      let i = 0;
      while (await db.user.findUnique({ where: { username: baseUsername + (i ? `${i}` : '') } })) {
        i++;
      }
    ...
    res.redirect(`${RootUrl}/google-auth-success?token=${session.id}&redirect_to=${redirTo}&username=${user.username}`);
  });
